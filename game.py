...
while True:
    ...
    tie = False
    engine = pyttsx3.init()
    s = "!!!! Enter 'start game' to start the game"
    print(s)
    engine.say(s)
    q = "!!!! Enter 'quit' to exit the game"
    print(q)
    engine.say(q)
    engine.runAndWait()
    ...
    if input_1 == 'exit' or input_1 == 'quit':
        engine = pyttsx3.init()
        r = "Open the recording live window and click 'q' on your keyboard to stop recording"
        print(">>",r)
        engine.say(r)
        engine.runAndWait()
        print(">> Recorded video will be in the same folder of this file with the file-name as 'game_rec.avi'")
        data = "file:///C:/Users/HI/Desktop/ML_2021/L5_Project/game_rec.avi"
        filename = "game_qr.png"
        img = qrcode.make(data)
        img.save(filename)
        print(">> You can also see the QR code for that video in the same folder with the file-name as 'game_qr.png")
        exit()
    elif input_1 == 'start game' or input_1 == 'new game' or input_1 == 'game play' or input_1 == 'game start':
            print(">> Your game is being to be recorded.")
